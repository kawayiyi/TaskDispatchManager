    ...
    public class QuartzHelper
    {
            ...
            try
            {
                lock (obj)
                {
                    if (scheduler == null)
                    {
                        ...
                        scheduler.Clear();
                        LogHelper.WriteInfoLog("任务调度初始化成功！");
                    }
                }
            }
            catch (Exception ex)
            {
                LogHelper.WriteErrorLog("任务调度初始化失败！", ex);
            }
        }
                ...
                if (!scheduler.IsStarted)
                {
                    ...
                    if (listTask != null && listTask.Count > 0)
                    {
                        foreach (TaskUtil taskUtil in listTask)
                        {
                            ...
                            {
                                LogHelper.WriteErrorLog(string.Format("任务“{0}”启动失败！", taskUtil.TaskName), e);
                            }
                        }
                    }
                    LogHelper.WriteInfoLog("任务调度启动成功！");
                }
            }
            catch (Exception ex)
            {
                LogHelper.WriteErrorLog("任务调度启动失败！", ex);
            }
        }
        ...
        public static void ScheduleJob(TaskUtil taskUtil)
        {
            ...
            if (ValidExpression(taskUtil.CronExpressionString))
            {
                ...
                LogHelper.WriteInfoLog(string.Format("任务“{0}”启动成功,未来5次运行时间如下:", taskUtil.TaskName));
                List<DateTime> list = GetTaskeFireTime(taskUtil.CronExpressionString, 5);
                foreach (var time in list)
                {
                    LogHelper.WriteInfoLog(time.ToString());
                }
            }
            else
            {
        ...
        public static void StopSchedule()
        {
            try
            {
                ...
                {
                    scheduler.Shutdown();
                    LogHelper.WriteInfoLog("任务调度停止！");
                }
            }
            catch (Exception ex)
            {
                LogHelper.WriteErrorLog("任务调度停止失败！", ex);
            }
        }
